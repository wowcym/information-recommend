...
# 这里输入需要查询的uid
file_name = "../Infor_Recommendation/data1_5w.csv"
...
def recommend_IC(uid):
    ...
    recommend = item_cf.recommend(uid, N=N, K=K)
    return recommend
...
def recommend_UC(uid):
    ...
    recommend = user_cf.recommend(uid, N=N, K=K)
    return recommend
...
def recommend_Hidden(uid):
    ...
    list_up = list(zip(recommend.keys(), recommend.values))
    return list_up
